...
class SSAST_v2(BaseSSAST):
    def __init__(self, 
                 ...
                 decoder_num_heads=6,
                 norm_layer=layernorm_wrapper) -> None:
        super().__init__(img_size, patch_size, in_chans, embed_dim, depth, num_heads, mlp_ratio, mask_ratio, masking_mode, use_cls_token, frequency_first, norm_layer)
        self.blocks = nn.ModuleList()
        for i in range(depth):
            self.blocks.append(
        ...
        pred.append(norm_layer(decoder_embed_dim))
        # add final output layer
        pred.append(nn.Sequential(nn.Linear(decoder_embed_dim, decoder_embed_dim), nn.GELU(), nn.Linear(decoder_embed_dim, self.img_patch_dim())))
...
def _get_ssast(encoder_name, **kwargs):
    ...
    use_v2_decoder = kwargs.pop("use_v2_decoder", False)
    if use_v2_decoder:
        ...
        return SSAST_v2(img_size=img_size, patch_size=patch_size, frequency_first=frequency_first, 
                        decoder_depth=decoder_depth, decoder_embed_dim=decoder_embed_dim, decoder_num_heads=decoder_num_heads,
                        **enc_params, **kwargs)
    else:
